...
public class LocationActivity2 extends FragmentActivity implements
    ...
    private void evictAllBut(final Marker marker) {
        final Driver driver = driverMapCache.get(marker.getId());
        if (driver != null) {
            googleMap.clear();
            addMarker(driver, driver.name, marker.getPosition().latitude, marker.getPosition().longitude).showInfoWindow();
            animateTo(marker.getPosition(), 15, 1, 1, 1000);
        }
    }
