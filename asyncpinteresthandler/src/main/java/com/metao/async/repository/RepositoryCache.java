...
public class RepositoryCache<K, V> {
     ...
     */
    public RepositoryCache(int maxSize) {
        if (maxSize <= 0) {
    ...
    public synchronized final String toString() {
        ...
        return String.format("RepositoryCache[maxSize=%d,hits=%d,misses=%d,hitRate=%d%%]",
                maxSize, hitCount, missCount, hitPercent);
    }

    public List<V> searchForValues(K key) {
        List<V> vList = new ArrayList<>();
        if (map.keys().hasMoreElements()) {
            K k = map.keys().nextElement();
            if (k == key) {
                vList.add(map.get(key));
            }
        }
        return vList;
    }
